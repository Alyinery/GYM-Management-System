<template>
    <div v-if="exist_data">
        ...
        Date(item.bookStartTime).getMonth() + 1}/${new Date(item.bookStartTime).getDate()}` }}</div>
                    ...
                    <div class="btn">
                        <van-button type="primary" color="#42b983" size="mini">进馆登记</van-button>
                    </div>
                </div>
            </div>
        </div>
...
export default {
    setup() {
        ...
        onMounted(() => {
            ...
            console.log(UserId)
            axios.get('http://localhost:8081/gym/booking/getBookedListByUserId', { params: { user_id: UserId } })
                .then(response => {
                    if (response.data.code === 20041) {
                        bookedInfoList.value = response.data.data;
                        console.log("BookedInfoList", bookedInfoList.value);
        ...
        return { goToPage, onClickLeft, bookedInfoList, exist_data, showCenter,showPopup,currentItem }
    }
}
</script>
