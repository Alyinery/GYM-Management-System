<template>
    <div v-if="exist_data">
        <div class="bookingList">
            <div class="items" v-for="item in bookingInfoList" :key="item.id">
                <div class="item" v-if="item.state === '0'">
        ...
        Date(item.rentStartTime).getMonth() + 1}/${new Date(item.rentStartTime).getDate()}` }}
                    ...
                    <div class="btn">
                        <van-button v-if="item.state === '0'" type="primary" color="#FF9933"
                            size="mini" @click="returnEquip(item.id,item.equipment_id)">立即归还</van-button>
                    </div>
                </div>
            </div>
        </div>
    ...
    <van-popup v-model:show="showCenter" round :style="{ padding: '10px' }">
        <div v-if="currentItem">
            ...
            租借状态：
            <span v-if="currentItem.state === '0'">租借中</span>
    ...
    setup() {
        ...
        const returnEquip = (id,equipId) => {
            showConfirmDialog({
                ...
                axios.post('http://localhost:8081/gym/booking/returnEquipBookInfoById',null,{params:{bookId:id,equip_id:equipId}})
                    .then(response => {
        ...
        return { onClickLeft, goToPage, onClickTab, exist_data, bookingInfoList, showPopup, currentItem, showCenter,returnEquip }
    }
}
</script>
