<template>
    <div class="world">
        <div class="head">
            <van-nav-bar color="#44A248" title="管理员注册" left-text="返回" left-arrow @click-left="onClickLeft" />
        </div>
        <div class="serve shadow">
            <div class="theme">Register</div>
            <van-form @submit="onSubmit">
                <van-cell-group inset>
                    <van-field v-model="username" name="用户名" label="用户名" placeholder="请输入管理员用户名"
                        :rules="userFormRules.userName" required :error-message="username_exist" />
                    <van-field v-model="password" type="password" name="密码" label="密码" placeholder="请输入管理员密码"
            ...
            <p class="login" @click="goToPage('/adminlogin')">已有帐号,去登陆</p>
        </div>
    </div>
</template>
<script>
    ...
    setup() {
        ...
        const userFormRules = ref({
            userName: [{ required: true, message: "请输入用户名" },
            {
                validator: async (newVal) => {
                    try {
                        const response = await axios.get('http://localhost:8081/gym/checkAdminUsername', {
        ...
        const onSubmit = async (values) => {
            ...
            if (canregister.value) {
                ...
                axios.post('http://localhost:8081/gym/adminRegister', data)
                    .then(response => {
                        // 处理后端返回的数据
                        if (response.data.code === 20041) {
                            //注册成功
        ...
        return { onClickLeft, goToPage, username, password, username_exist, onSubmit, userFormRules, canclick }
    }
}
</script>
